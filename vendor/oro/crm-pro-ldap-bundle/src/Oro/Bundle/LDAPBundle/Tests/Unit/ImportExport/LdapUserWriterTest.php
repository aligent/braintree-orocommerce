...
use Oro\Bundle\LDAPBundle\ImportExport\LdapHelper;
...
class LdapUserWriterTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->contextMediator->expects($this->any())
            ...
            ->will(
                $this->returnValue(
                    $this->transport = $this->createMock(
                        'Oro\Bundle\LDAPBundle\Provider\Transport\LdapTransportInterface'
                    )
                )
            );

        $this->helper = $this->getMockBuilder('Oro\Bundle\LDAPBundle\ImportExport\LdapHelper')
            ->disableOriginalConstructor()
            ->getMock();
