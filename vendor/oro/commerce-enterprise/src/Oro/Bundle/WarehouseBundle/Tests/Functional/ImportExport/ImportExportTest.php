 ...
 */
class ImportExportTest extends BaseImportExportCase
{
    /**
     * @var array
     */
    ...
    public function testExportDetailedInventoryLevel()
    {
        ...
        $unitKey = array_search('Unit', $fileContent[0]);
        $this->assertGreaterThanOrEqual(0, $unitKey);

        // check correct unit format
        $exportUnits = [];
        for ($i = 1; $i < count($fileContent); $i++) {
            $exportUnits[] = $fileContent[$i][$unitKey];
        }
