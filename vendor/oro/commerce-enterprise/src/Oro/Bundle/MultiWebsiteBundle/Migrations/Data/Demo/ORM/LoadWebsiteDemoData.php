...
namespace Oro\Bundle\MultiWebsiteBundle\Migrations\Data\Demo\ORM;
...
class LoadWebsiteDemoData extends AbstractFixture implements ContainerAwareInterface, DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        foreach ($websites as $url => $website) {
            $configManager->set('oro_website.url', $url, $website->getId());
            $configManager->set('oro_website.secure_url', $url, $website->getId());
            $configManager->flush($website->getId());
        }
    ...
    {
        $website = $manager->getRepository('OroWebsiteBundle:Website')->findOneBy(['name' => $name]);
