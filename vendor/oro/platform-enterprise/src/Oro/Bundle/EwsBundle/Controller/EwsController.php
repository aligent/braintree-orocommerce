...
namespace Oro\Bundle\EwsBundle\Controller;
...
use Oro\Bundle\EwsBundle\Ews\EwsType\DistinguishedFolderIdNameType;

/**
 * Class EwsController
 * @package Oro\Bundle\EwsBundle\Controller
 */
class EwsController extends Controller
{
    /**
     * @Route("/ping", name="oro_ews_ping")
     */
    public function pingAction()
    {
        ...
        try {
            $ewsConnector = $this->get('oro_ews.transport_check.connector');
            $defaultConfigurator = $this->get('oro_ews.service_configurator');
            ...
            $configurator = $this->get('oro_ews.transport_check.service_configurator');
            ...
            if (!empty($response) && isset($response[0]) && !empty($response[0]->RootFolder)) {
                $result = ['msg' => $this->get('translator')->trans('oro_ews.controller.message.ok')];
            } else {
                $result = ['error' => $this->get('translator')->trans('oro_ews.controller.message.error')];
            }
