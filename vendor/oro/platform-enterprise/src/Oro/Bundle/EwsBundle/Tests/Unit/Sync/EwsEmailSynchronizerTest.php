...
use Oro\Bundle\EwsBundle\Tests\Unit\Sync\Fixtures\TestEwsEmailSynchronizer;
...
class EwsEmailSynchronizerTest extends OrmTestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->emailAddressManager->expects($this->any())
            ...
            ->will($this->returnValue('Oro\Bundle\EwsBundle\Tests\Unit\Sync\Fixtures\Entity\TestEmailAddress'));
        $emailOwnerProviderStorage = new EmailOwnerProviderStorage();
        $userEmailOwnerProvider = $this
            ->createMock('Oro\Bundle\EmailBundle\Entity\Provider\EmailOwnerProviderInterface');
        ...
        $ewsConnector = $this->getMockBuilder('Oro\Bundle\EwsBundle\Connector\EwsConnector')
            ->disableOriginalConstructor()
            ->getMock();
        $this->ewsConfigurator = $this->getMockBuilder('Oro\Bundle\EwsBundle\Provider\EwsServiceConfigurator')
            ->disableOriginalConstructor()
            ->getMock();
    ...
    protected function initializeEntityManager()
    {
        ...
        $oroProBasePath = dirname((new \ReflectionClass(new OroEwsBundle()))->getFileName()) . '/..';
    ...
    public function testCreateSynchronizationProcessor()
    {
        ...
        $this->assertInstanceOf(
            'Oro\Bundle\EwsBundle\Sync\EwsEmailSynchronizationProcessor',
            $processor
        );
    }
    ...
    public function testInitializeOrigins()
    {
        ...
        $selectStmt->expects($this->at(1))
            ->method('bindValue')
            ->with(2, 'test_server');
        $insertOriginStmt = $this->createMock('Oro\Bundle\TestFrameworkBundle\Test\Doctrine\ORM\Mocks\StatementMock');
