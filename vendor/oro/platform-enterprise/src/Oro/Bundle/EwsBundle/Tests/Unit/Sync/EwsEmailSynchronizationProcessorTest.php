...
use Oro\Bundle\EwsBundle\Entity\EwsEmail;
...
use Oro\Bundle\EwsBundle\Sync\EwsEmailSynchronizationProcessor;
use Oro\Bundle\EwsBundle\Sync\FolderInfo;
...
class EwsEmailSynchronizationProcessorTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->knownEmailAddressChecker =
            $this->getMockBuilder('Oro\Bundle\EmailBundle\Sync\KnownEmailAddressCheckerInterface')
                ->disableOriginalConstructor()
                ->getMock();
        $this->manager = $this->getMockBuilder('Oro\Bundle\EwsBundle\Manager\EwsEmailManager')
            ->disableOriginalConstructor()
            ->getMock();
    }
    ...
    public function testSyncFolders()
    {
        ...
        $folders = [];
        $folders['1'] = new FolderInfo($ewsFolder1, false);
        $folders['2'] = new FolderInfo($ewsFolder2, false);
        $folders['3'] = new FolderInfo($ewsFolder3, false);

        $repo = $this->getMockBuilder('Oro\Bundle\EwsBundle\Entity\Repository\EwsEmailFolderRepository')
            ->disableOriginalConstructor()
            ->getMock();
        ...
        $this->em->expects($this->once())
            ...
            ->with('OroEwsBundle:EwsEmailFolder')
    ...
    public function testProcess()
    {
        ...
        $sqb = $this->getMockBuilder('Oro\Bundle\EwsBundle\Connector\Search\SearchQueryBuilder')
            ->disableOriginalConstructor()
            ->getMock();
        $sq = $this->getMockBuilder('Oro\Bundle\EwsBundle\Connector\Search\SearchQuery')
            ->disableOriginalConstructor()
            ->getMock();
    ...
    public function testSaveEmails()
    {
        ...
        $repo = $this->getMockBuilder('Oro\Bundle\EwsBundle\Entity\Repository\EwsEmailRepository')
            ->disableOriginalConstructor()
            ->getMock();
    ...
    {
        $processor = $this->getMockBuilder('Oro\Bundle\EwsBundle\Sync\EwsEmailSynchronizationProcessor')
            ->setConstructorArgs(
                [
                    $this->em,
                    $this->emailEntityBuilder,
