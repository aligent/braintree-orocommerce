...
class OverrideEntityAclExtensionPass implements CompilerPassInterface
{
    ...
    const SYSTEM_MODE_ORG_PROVIDER = 'oro_organizationpro.system_mode_org_provider';
    ...
    const USER_ACL_HANDLER_CLASS   = 'Oro\Bundle\SecurityProBundle\Autocomplete\UserAclProHandler';

    const USER_ACL_GRID_LISTENER_SERVICE = 'oro_user.event_listener.owner_user_grid_listener';
    const USER_ACL_GRID_LISTENER_CLASS   = 'Oro\Bundle\SecurityProBundle\EventListener\OwnerUserProGridListener';

    const SQL_WALKER_BUILDER_SERVICE = 'oro_security.orm.ownership_sql_walker_builder';
    const SQL_WALKER_BUILDER_CLASS   = 'Oro\Bundle\SecurityProBundle\ORM\Walker\OwnershipProConditionDataBuilder';

    const PARAM_CONVERTER_SERVICE     = 'sensio_framework_extra.converter.doctrine.orm';
    const PARAM_CONVERTER_CLASS       = 'Oro\Bundle\SecurityProBundle\Request\ParamConverter\DoctrineParamProConverter';
    ...
    const IMPORTEXPORT_ENTITY_READER_CLASS   = 'Oro\Bundle\SecurityProBundle\ImportExport\Reader\EntityProReader';

    const OWNERSHIP_METADATA_SERVICE = 'oro_security.owner.ownership_metadata_provider';
    const OWNERSHIP_METADATA_CLASS   = 'Oro\Bundle\SecurityProBundle\Owner\Metadata\OwnershipMetadataProProvider';

    const ORM_ACL_HELPER_SERVICE = 'oro_security.acl_helper';
    const NEW_ORM_ACL_HELPER_CLASS = 'Oro\Bundle\SecurityProBundle\ORM\Walker\AclHelper';
    const ORM_SHARE_CONDITION_DATA_BUILDER_SERVICE = 'oro_securitypro.orm.share_condition_data_builder';
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        if ($container->hasDefinition(self::SEARCH_ACL_HELPER)) {
            ...
            $definition->addMethodCall('setOrganizationProHelper', [new Reference('oro_organizationpro.helper')]);
