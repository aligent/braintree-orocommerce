...
namespace Oro\Bundle\SecurityProBundle\Model\Action;
...
use Oro\Bundle\SecurityProBundle\Exception\UnknownShareScopeException;
...
class AceQueryManager implements AceQueryInterface
{
    ...
    {
        $qb = $this->doctrineHelper->getEntityRepository('OroSecurityProBundle:AclEntry')->createQueryBuilder('ae')
            ->delete();
        $qb->where('ae.class = :class');

        $qbSub = $this->doctrineHelper->getEntityRepository('OroSecurityProBundle:AclSecurityIdentity')
