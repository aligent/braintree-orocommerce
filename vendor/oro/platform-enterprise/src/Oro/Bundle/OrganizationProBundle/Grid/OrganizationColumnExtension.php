...
namespace Oro\Bundle\OrganizationProBundle\Grid;
...
class OrganizationColumnExtension extends AbstractExtension
{
    ...
    protected function getEntity(DatagridConfiguration $config)
    {
        if (null === $this->entityClassName) {
            $this->entityClassName = $config->getExtendedEntityClassName();
            if (!$this->entityClassName) {
                $this->entityClassName = $config->getOrmQuery()->getRootEntity($this->entityClassResolver);
            }
        }
