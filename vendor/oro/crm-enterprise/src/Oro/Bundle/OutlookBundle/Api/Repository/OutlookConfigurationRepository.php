...
namespace Oro\Bundle\OutlookBundle\Api\Repository;
...
class OutlookConfigurationRepository
{
    ...
    public function getOutlookSectionOptions($scope)
    {
        ...
        $result[] = $this->createConfigurationOption(
            $scope,
            'oro_outlook.addin_latest_version',
            'string',
            $addInLatestVersion
        );
        if ($addInLatestVersion) {
            ...
            $result[] = $this->createConfigurationOption(
                $scope,
                'oro_outlook.addin_latest_version_url',
                'string',
            ...
            if (!empty($file['doc_url'])) {
                $result[] = $this->createConfigurationOption(
                    $scope,
                    'oro_outlook.addin_latest_version_doc_url',
                    'string',
            ...
            if ($minSupportedVersion) {
                $result[] = $this->createConfigurationOption(
                    $scope,
                    'oro_outlook.addin_min_supported_version',
                    'string',
                    $minSupportedVersion
                );
            }
